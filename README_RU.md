...
## Зачем появился этот форк

Я хотел ридер именно с таким сценарием перевода и даже думал писать приложение с нуля.  
Потом нашел **Anx Reader** — он очень понравился. Он уже был близок к тому, что мне нужно, но не совпадал полностью.  
Поэтому я сделал этот форк и довел его до нужного мне варианта с помощью **Antigravity** и **Codex**.

## Что добавлено в форке

- ⚡ Пакетный AI-перевод для более быстрой обработки страниц/глав
- 📘 Надстрочный пословный режим с маркерами вида `[word|translation|min_level]`
- 🎯 Фильтрация по CEFR-уровням (`0`, `A1`, `A2`, `B1`, `B2`, `C1`, `C2`) — показываются только нужные подсказки
- 🎨 Опциональная раскраска перевода по уровням
- 🧩 Настраиваемая палитра цветов для каждого CEFR-уровня (сброс к дефолтной)
- 🧠 Кэш переводов (уровень предложений + отдельный словарный кэш)
- 🧹 Очистка кэша по области (текущая страница / глава / вся книга)
- 🌍 Переопределение исходного языка для книги + автоопределение по текущей главе
- 🛠 Опциональный AI-refine CEFR-уровней для не-AI переводчиков (глобально и для конкретной книги)
- 📊 Оверлей статуса AI с логами/историей запросов
- 🔁 Ретраи и backoff при 429
- ⚙️ Настройка размера batch и количества workers

## Скриншот

![Пример надстрочного перевода](./docs/images/interlinear-pt.png)

> ⚠️ **Статус форка:** версия пока ранняя и местами может быть нестабильной.  
> В первую очередь я доводил ее до состояния “достаточно хорошо для моего повседневного использования”.  
> Публикую проект, чтобы не потерять сам концепт и реализацию.  
> Если будут проблемы — буду допиливать и исправлять.
